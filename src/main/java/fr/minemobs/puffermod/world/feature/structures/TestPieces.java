...
public class TestPieces {
    ...
    private static final ResourceLocation PART_1 = new ResourceLocation(Main.modId, "test_structure");

    private static final Map<ResourceLocation, BlockPos> OFFSET = ImmutableMap.of(PART_1, new BlockPos(0, 1,0));

    public static void start(TemplateManager manager, BlockPos pos, Rotation rot, List<StructurePiece> pieces, SharedSeedRandom rand) {
        int x = pos.getX();
        int z = pos.getZ();

        BlockPos rotationOffset = new BlockPos(0, 0, 0).rotate(rot);
        BlockPos blockPos = rotationOffset.add(x, pos.getY(), z);
        pieces.add(new TestPieces.Piece(manager, PART_1, blockPos, rot));
    }
    ...
    public static class Piece extends TemplateStructurePiece {
        ...
        public Piece(TemplateManager templateManagerIn, ResourceLocation resourceLocationIn, BlockPos pos,
                     Rotation rotationIn) {
            super(FeatureInit.TEST_PIECE, 0);
            this.resourceLocation = resourceLocationIn;
            BlockPos blockpos = TestPieces.OFFSET.get(resourceLocation);
        ...
        public Piece(TemplateManager templateManagerIn, CompoundNBT tagCompound) {
            super(FeatureInit.TEST_PIECE, tagCompound);
        ...
        public boolean create (IWorld worldIn, ChunkGenerator<?> generator, Random randomIn,
            ...
            PlacementSettings placementsettings = (new PlacementSettings()).setRotation(this.rotation)
                    .setMirror(Mirror.NONE);
            BlockPos blockpos = TestPieces.OFFSET.get(this.resourceLocation);
